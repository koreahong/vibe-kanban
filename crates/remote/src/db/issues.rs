...
impl IssueRepository {
    ...
    pub async fn create(
        ...
        // unless pre-set (e.g., from Jira sync with a Jira key like "MPD-172")
        let data: Issue = sqlx::query_as(
            r#"
            INSERT INTO issues (
                id, project_id, status_id, title, description, priority,
                simple_id, issue_number,
                start_date, target_date, completed_at, sort_order,
                parent_issue_id, parent_issue_sort_order, extension_metadata,
                creator_user_id
            )
            VALUES ($1, $2, $3, $4, $5, $6, COALESCE(NULLIF($7, ''), ''), COALESCE($8, 0), $9, $10, $11, $12, $13, $14, $15, $16)
            RETURNING *
            "#,
        )
        .bind(id)
